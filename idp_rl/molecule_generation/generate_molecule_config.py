...
def config_from_molFile(file: str, num_conformers: int, calc_normalizers: bool = False, pruning_thresh: float = 0.05, save_file: str = "") -> MolConfig:
    """Generates a :class:`~idp_rl.config.mol_config.MolConfig` object for a molecule specified by the location of a 
    ...
    ----------
    file : str
        Name of the MOL file containing the molecule to be converted into a :class:`~idp_rl.config.mol_config.MolConfig` object.
    ...
    calc_normalizers : bool
        ...
        See :class:`~idp_rl.config.mol_config.MolConfig` for more details.
    ...
    Returns
    -------
    :class:`~idp_rl.config.mol_config.MolConfig`
        A :class:`~idp_rl.config.mol_config.MolConfig` object configured with the input molecule, as well as
...
def config_from_smiles(smiles: str, num_conformers: int, calc_normalizers: bool = False, pruning_thresh: float = 0.05, save_file: str = "") -> MolConfig:
    """Generates a :class:`~idp_rl.config.mol_config.MolConfig` object for a molecule specified by a 
