...
class ExternalCurriculumAgentMixin():
    ...
    Parameters
    ----------
    config : :class:`~idp_rl.config.agent_config.Config`
    ...
    requires the following additional parameters in the :class:`~idp_rl.config.agent_config.Config` object:
    ...
    * curriculum_agent_fail_rate
    ...
    def update_curriculum(self) -> None:
        ...
        has exceeded the parameter ``curriculum_agent_buffer_len`` assigned in the :class:`~idp_rl.config.agent_config.Config` object.
        ...
        the ``curriculum_agent_reward_thresh`` parameter defined in the :class:`~idp_rl.config.agent_config.Config`
...
from idp_rl.agents import PPOAgent, PPORecurrentAgent
from idp_rl.agents import A2CAgent, A2CRecurrentAgent

class PPOExternalCurriculumAgent(ExternalCurriculumAgentMixin, PPOAgent):
    """Implementation of :mod:`~idp_rl.agents.PPO.PPO_agent.PPOAgent` compatible with
    environments that use curriculum learning. See :meth:`~idp_rl.agents.curriculum_agents.ExternalCurriculumAgentMixin.update_curriculum`
    for more details.
...
class PPORecurrentExternalCurriculumAgent(ExternalCurriculumAgentMixin, PPORecurrentAgent):
    """Implementation of :mod:`~idp_rl.agents.PPO.PPO_recurrent_agent.PPORecurrentAgent` compatible with
    environments that use curriculum learning. See :meth:`~idp_rl.agents.curriculum_agents.ExternalCurriculumAgentMixin.update_curriculum`
    for more details.
...
class A2CExternalCurriculumAgent(ExternalCurriculumAgentMixin, A2CAgent):
    """Implementation of :mod:`~idp_rl.agents.A2C.A2C_agent.A2CAgent` compatible with
    environments that use curriculum learning. See :meth:`~idp_rl.agents.curriculum_agents.ExternalCurriculumAgentMixin.update_curriculum`
    for more details.
...
class A2CRecurrentExternalCurriculumAgent(ExternalCurriculumAgentMixin, A2CRecurrentAgent):
    """Implementation of :mod:`~idp_rl.agents.A2C.A2C_recurrent_agent.A2CRecurrentAgent` compatible with
    environments that use curriculum learning. See :meth:`~idp_rl.agents.curriculum_agents.ExternalCurriculumAgentMixin.update_curriculum`
    for more details.
