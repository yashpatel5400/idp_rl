...
def load_data_from_pickle(paths: List[str], indices: Optional[List[str]]=None) -> dict:
    ...
    Notes
    -----
    The ``.pickle`` files specified by `paths` should be dumped directly by :class:`~idp_rl.logging.env_logger.EnvLogger`,
    and should correspond to a single evaluation episode.
    See :meth:`idp_rl.logging.env_logger.EnvLogger.save_episode` for more details on the dumped format.
