...
class RTGNGatRecurrent(torch.nn.Module):
    ...
    and long short-term memory (LSTM) for predicting discrete torsion angles.
    ...
    def __init__(self, action_dim: int, hidden_dim: int, node_dim: int):
        ...
        self.actor = _RTGNGatActorRecurrent(action_dim, hidden_dim, node_dim=node_dim)
        self.critic = _RTGNGatCriticRecurrent(action_dim, hidden_dim, node_dim=node_dim)
...
class _RTGNGatCriticRecurrent(torch.nn.Module):
...
class _RTGNGatActorRecurrent(torch.nn.Module):
    def __init__(self, action_dim, hidden_dim, node_dim):
        super().__init__()
        self.gat = GAT(hidden_dim=hidden_dim, node_dim=node_dim)
        self.set2set = gnn.Set2Set(hidden_dim, processing_steps=6)
